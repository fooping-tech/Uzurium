...
void NEOPIXELLED::setup(int pin){
  //良い解決法求む
  ...
  switch(pin){
    ...
    case 32:
      ...
      break;
    default:
      Serial.println("NEOPIXELLED Unsupported Pin");
      break;
  }
  FastLED.setBrightness(25);
  Serial.println("NEOPIXELLED setup was completed.");
}
void NEOPIXELLED::setbrightness(int vol){
...
void NEOPIXELLED::fire2(int mode,int hue){
...
void NEOPIXELLED::inspection(int color){
  if(color==0){
    for(int i = 0; i < NUM_LEDS; i++) {
      leds[i] = CRGB(0 , 0, 255);
    }
...
void NEOPIXELLED::counter(int count){
...
void NEOPIXELLED::level(int count){
  for(int i = 0; i < NUM_LEDS;i++){
...
void NEOPIXELLED::level2(int count1,int count2){
  for(int i = 0; i < NUM_LEDS/2;i++){
    if(0<=i && i<count1)leds[i] = CHSV(i*10,255,255);
    if(count1<=i && i<NUM_LEDS)leds[i] = CHSV(0,0,0);
	}
  for(int i = NUM_LEDS; i >= NUM_LEDS/2;i--){
    if(NUM_LEDS - count2<=i && i<NUM_LEDS)leds[i] = CHSV((NUM_LEDS - i)*10,255,255);
    if(NUM_LEDS/2<=i && i<NUM_LEDS - count2)leds[i] = CHSV(0,0,0);
	}
  FastLED.show(); 
}
void NEOPIXELLED::fade(){
  FastLED.show(); 
  fadeall();
  FastLED.delay(1000 / FPS);
}
void NEOPIXELLED::clyon(){
...
void NEOPIXELLED::flash(int led_color){
  for(int i=0;i<NUM_LEDS;i++){
